...
struct add {
    constexpr void
    operator()(svm::vm_state& state, tvm::reg_ind_t<4> r1, tvm::reg_ind_t<4> r2) {
        state[r1] += state[r2];
    }
};
...
struct movi {
    constexpr void
    operator()(svm::vm_state& state, tvm::reg_ind_t<> r1, tvm::operand_t<16> op) {
        state[r1] = (uint16_t)op.operand;
    }
};
...
struct push {
    constexpr void operator()(svm::vm_state& state, tvm::reg_ind_t<4> reg) {
        ...
        *state.stack_cur++ = reg.index == 15 ? state.pc : state[reg];
    }
};
...
struct read_byte {
    void operator()(svm::vm_state& state, tvm::reg_ind_t<4> to, tvm::reg_ind_t<4> reg) {
        auto ptr = reinterpret_cast<uint8_t*>(state[reg]);
        state[to] = *ptr;
    }
};
...
struct read_word {
    void operator()(svm::vm_state& state, tvm::reg_ind_t<4> to, tvm::reg_ind_t<4> reg) {
        auto ptr = reinterpret_cast<uint16_t*>(state[reg]);
        state[to] = *ptr;
    }
};
