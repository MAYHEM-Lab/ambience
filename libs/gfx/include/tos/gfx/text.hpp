...
namespace tos::gfx
{
struct character : public fixed_canvas<8, 8>
{
public:
    explicit constexpr character(const std::array<uint8_t, 8>& buf) {
    ...
    constexpr character transposed() const {
        fixed_canvas<8, 8> self{};
        for (int row = 0; row < 8; ++row)
        {
            self.set_word(0, row, get_word(0, row));
        }

        fixed_canvas<8, 8> out{};
...
public:
    ...
    constexpr auto transform(FunT&& fn) const {
        return charset{ map([&](auto x) {
            return std::pair<char, character>(x.first, (x.second.*fn)());
        }, m_chars) };
    }
...
constexpr auto basic_font()
{
    ...
    constexpr character c0 {{ 0x3E, 0x63, 0x73, 0x7B, 0x6F, 0x67, 0x3E, 0x00}};
    constexpr character c1 {{ 0x0C, 0x0E, 0x0C, 0x0C, 0x0C, 0x0C, 0x3F, 0x00}};
    constexpr character c2 {{ 0x1E, 0x33, 0x30, 0x1C, 0x06, 0x33, 0x3F, 0x00}};
    constexpr character c3 {{ 0x1E, 0x33, 0x30, 0x1C, 0x30, 0x33, 0x1E, 0x00}};
    constexpr character c4 {{ 0x38, 0x3C, 0x36, 0x33, 0x7F, 0x30, 0x78, 0x00}};
    constexpr character c5 {{ 0x3F, 0x03, 0x1F, 0x30, 0x30, 0x33, 0x1E, 0x00}};
    constexpr character c6 {{ 0x1C, 0x06, 0x03, 0x1F, 0x33, 0x33, 0x1E, 0x00}};
    constexpr character c7 {{ 0x3F, 0x33, 0x30, 0x18, 0x0C, 0x0C, 0x0C, 0x00}};
    constexpr character c8 {{ 0x1E, 0x33, 0x33, 0x1E, 0x33, 0x33, 0x1E, 0x00}};
    constexpr character c9 {{ 0x1E, 0x33, 0x33, 0x3E, 0x30, 0x18, 0x0E, 0x00}};

    constexpr character colon {{ 0x00, 0x0C, 0x0C, 0x00, 0x00, 0x0C, 0x0C, 0x00}};
    constexpr character dash { { 0x00, 0x00, 0x00, 0x3F, 0x00, 0x00, 0x00, 0x00}};
    constexpr character slash {{ 0x60, 0x30, 0x18, 0x0C, 0x06, 0x03, 0x01, 0x00}};

    return charset<68>(frozen::make_unordered_map<char, character>({
        { 'A', A },
        { 'B', B },
        { 'C', C },
        { 'D', D },
