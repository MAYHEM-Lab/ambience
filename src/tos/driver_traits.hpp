...
namespace tos
{
    namespace details {
        ...
        struct read_impl_exists<void, std::void_t<decltype(std::declval<Arg>().read( std::declval<Args>()...) )>, Arg, Args...>
        : std::true_type {};

        template <class, class, class... Args>
        struct write_impl_exists : std::false_type {};
        template <class Arg, class... Args>
        struct write_impl_exists<void, std::void_t<decltype(std::declval<Arg>().write( std::declval<Args>()...) )>, Arg, Args...>
                : std::true_type {};
    }
    ...
    {
        using has_read = read_impl_exists<T, tos::span<char>>;
        using has_write = write_impl_exists<T, tos::span<const char>>;
    };
