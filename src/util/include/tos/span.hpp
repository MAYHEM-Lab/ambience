...
public:
    ...
    constexpr T& operator[](size_t ind) { return m_base[ind]; }

    constexpr const T& operator[](size_t ind) const { return m_base[ind]; }
     ...
     */
    constexpr operator span<const T>() const { return {m_base, size_t(m_len)}; }
     ...
     * @return a new span
     */
    constexpr span slice(size_t begin, size_t len) { return {m_base + begin, len}; }
     ...
     * @return a new span
     */
    constexpr span slice(size_t begin) { return {m_base + begin, size() - begin}; }
