...
uart_config(uint8 uart_no) {
    ...
    if (uart_no == UART0) {
        ...
        WRITE_PERI_REG(UART_CONF1(uart_no),
                       ...
                       UART_RX_TOUT_EN |
                       ((1 & UART_TXFIFO_EMPTY_THRHD) << UART_TXFIFO_EMPTY_THRHD_S));

        SET_PERI_REG_MASK(UART_INT_ENA(uart_no), UART_RXFIFO_TOUT_INT_ENA | UART_FRM_ERR_INT_ENA);
    }
    else
    {
        WRITE_PERI_REG(UART_CONF1(uart_no), ((UartDev.rcv_buff.TrigLvl & UART_RXFIFO_FULL_THRHD)
...
static tos::mutex tx_busy;
static const char* volatile c;
static volatile size_t buf_sz;
static tos::semaphore tx_done{0};

STATUS uart_tx_one_char(uint8 uart, uint8 TxChar) {
    tos::lock_guard<tos::mutex> lk{tx_busy};
    c = (const char*)&TxChar;
    buf_sz = 1;
    SET_PERI_REG_MASK(UART_INT_ENA(uart), UART_TXFIFO_EMPTY_INT_ENA);
    CLEAR_PERI_REG_MASK(UART_INT_CLR(uart), UART_TXFIFO_EMPTY_INT_CLR);
    tx_done.down();
    return OK;
}
...
LOCAL void
uart0_rx_intr_handler(void *) {
    uint8 uart_no = UART0;
    auto st = READ_PERI_REG(UART_INT_ST(uart_no));

    if (UART_FRM_ERR_INT_ST == (st & UART_FRM_ERR_INT_ST))
    {
        WRITE_PERI_REG(UART_INT_CLR(uart_no), UART_FRM_ERR_INT_CLR);
    }
    if (UART_RXFIFO_FULL_INT_ST == (st & UART_RXFIFO_FULL_INT_ST))
    {
        uart_rx_intr_disable(UART0);
        WRITE_PERI_REG(UART_INT_CLR(UART0), UART_RXFIFO_FULL_INT_CLR);
        system_os_post(uart_recvTaskPrio, 0, 0);
    }
    if (UART_RXFIFO_TOUT_INT_ST == (st & UART_RXFIFO_TOUT_INT_ST))
    {
        uart_rx_intr_disable(UART0);
        WRITE_PERI_REG(UART_INT_CLR(UART0), UART_RXFIFO_TOUT_INT_CLR);
        system_os_post(uart_recvTaskPrio, 0, 0);
    }
    if (UART_TXFIFO_EMPTY_INT_ST == (st & UART_TXFIFO_EMPTY_INT_ST))
    {
        WRITE_PERI_REG(UART_INT_CLR(uart_no), UART_TXFIFO_EMPTY_INT_ST);
    ...
    if (UART_RXFIFO_OVF_INT_ST == (st & UART_RXFIFO_OVF_INT_ST))
    {
...
uart_sendTask(os_event_t *events) {
    ...
    if (uart_recv_line_cb != 0)
        uart_recv_line_cb((char *) uartReceiveBuffer);
    start_uart = 1;
    uart_busy = 0;
}
...
uart_init_2(UartBautRate uart0_br, UartBautRate uart1_br) {
    ...
    os_install_putc1(uart1_write_char);//print output at UART1
}
