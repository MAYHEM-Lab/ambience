...
{
    /**
     * NRF5 BLE SDK provides a way to store the large, 16 byte base UUIDs for GATT services etc in the soft device
     * memory, and access them through indices into this memory
     *
    ...
    public:
        constexpr explicit
        uuid_handle_t(uint8_t hndl) : m_handle{hndl} {}
    private:
     ...
     *
     * The SDK also allows the 12th and 13th bytes of such UUIDs to be provided alongside the handles to these
     * UUIDs so that characteristics for GATT services could be handled without allocating a new UUID
     *
