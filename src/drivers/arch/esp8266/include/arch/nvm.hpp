    ...
    namespace esp82
    {
        ...
        public:
            ...
            write(sector_id_t sector, tos::span<const uint8_t> data, size_t offset = 0)
            {
                ...
                auto res = spi_flash_write((uint32_t)sector * sector_size() + offset, (uint32*)data.data(), data.size());
                if (res == SPI_FLASH_RESULT_OK)
                {
                    return {};
                }
            ...
            read(sector_id_t sector, tos::span<uint8_t> buf, size_t offset = 0)
            {
                ...
                auto res = spi_flash_read((uint32_t)sector * sector_size() + offset, (uint32*)buf.data(), buf.size());
                if (res == SPI_FLASH_RESULT_OK)
                {
                    return buf;
                } else {
