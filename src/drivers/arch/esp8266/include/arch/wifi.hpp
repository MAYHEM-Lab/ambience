...
public:
    ...
    connect(span<const char> ssid) noexcept ICACHE_FLASH_ATTR {
        return connect(ssid, "");
    }
...
namespace devs {
template<int N>
using wifi_t = dev<struct _wifi_t, N>;
template<int N>
static constexpr wifi_t<N> wifi{};
} // namespace devs

inline esp82::wifi open_impl(devs::wifi_t<0>) { return {}; }
} // namespace tos