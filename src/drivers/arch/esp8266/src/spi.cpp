...
namespace tos {
namespace esp82 {
spi::spi(spi::gpio_type&) { spi_init(HSPI); }
...
uint8_t spi::exchange(uint8_t byte) {
...
//80MHz 									 sysclock for SPI clock. 									 SPI_CLK_USE_DIV (0) if using divider to 									 get lower SPI
//clock speed.
//
////////////////////////////////////////////////////////////////////////////////
...
void spi_init_gpio(uint8 spi_no, uint8 sysclk_as_spiclk) {
    ...
    if (spi_no == SPI) {
        WRITE_PERI_REG(PERIPHS_IO_MUX,
                       0x005 |
                           (clock_div_flag << 8)); // Set bit 8 if 80MHz sysclock required
        ...
        WRITE_PERI_REG(PERIPHS_IO_MUX,
                       0x105 |
                           (clock_div_flag << 9)); // Set bit 9 if 80MHz sysclock required
...
//				  Set either divider to 0 to disable all division (80MHz
//sysclock)
//
////////////////////////////////////////////////////////////////////////////////
...
//							   Data is sent out starting with Bit31 and
//down to Bit0
//
//							   SPI_BYTE_ORDER_LOW_TO_HIGH (0)
//							   Data is sent out starting with the lowest
//BYTE, from 							   MSB to LSB, followed by the second lowest BYTE, from 							   MSB to LSB, followed by
//the second highest BYTE, from 							   MSB to LSB, followed by the highest BYTE, from MSB to LSB
...
//    Parameters: spi_no - SPI (0) or HSPI (1)
//				  byte_order - SPI_BYTE_ORDER_HIGH_TO_LOW (1)
//							   Data is read in starting with Bit31 and
//down to Bit0
//
//							   SPI_BYTE_ORDER_LOW_TO_HIGH (0)
//							   Data is read in starting with the lowest
//BYTE, from 							   MSB to LSB, followed by the second lowest BYTE, from 							   MSB to LSB, followed by
//the second highest BYTE, from 							   MSB to LSB, followed by the highest BYTE, from MSB to LSB
...
//				  Note: all data is assumed to be stored in the lower bits
//of 				  the data variables (for anything <32 bits).
//
////////////////////////////////////////////////////////////////////////////////
...
uint32 spi_transaction(uint8 spi_no,
    ...
    CLEAR_PERI_REG_MASK(SPI_USER(spi_no),
                        SPI_USR_MOSI | SPI_USR_MISO | SPI_USR_COMMAND | SPI_USR_ADDR |
                            SPI_USR_DUMMY);
    ...
    if (dout_bits) {
        ...
        if (READ_PERI_REG(SPI_USER(spi_no)) & SPI_WR_BYTE_ORDER) {
            ...
            if (dout_extra_bits) {
                ...
                // SPI_WR_BYTE_ORDER set, you need this to move the non-8bit remainder to
                // the MSBs not sure if there's even a use case for this, but it's here if
                // you need it... for example, 0xDA4 12 bits without SPI_WR_BYTE_ORDER
                // would usually be output as if it were 0x0DA4, of which 0xA4, and then
                // 0x0 would be shifted out (first 8 bits of low byte, then 4 MSB bits of
                // high byte - ie reverse byte order). The code below shifts it out as 0xA4
                // followed by 0xD as you might require.
                WRITE_PERI_REG(
                    SPI_W0(spi_no),
    ...
    if (din_bits) {
        ...
        if (READ_PERI_REG(SPI_USER(spi_no)) & SPI_RD_BYTE_ORDER) {
            return READ_PERI_REG(SPI_W0(spi_no)) >>
                   (32 - din_bits); // Assuming data in is written to MSB. TBC
        } else {
            return READ_PERI_REG(
                SPI_W0(spi_no)); // Read in the same way as DOUT is sent. Note existing
                                 // contents of SPI_W0 remain unless overwritten!
        }
    }
    //########## END SECTION ##########//
