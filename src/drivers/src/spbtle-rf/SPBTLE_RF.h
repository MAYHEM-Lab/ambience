...
public:
    ...
    spbtle_rf(
        SpiT spi, ExtiT exti, tos::any_alarm& alarm, PinT cs, PinT irq_pin, PinT reset);

    tos::expected<tos::spbtle::gatt, errors> initialize_gatt();

    tos::spbtle::gap initialize_gap(tos::spbtle::gatt&, std::string_view name);

    bool data_present() const {
        return m_gpio->read(m_irq_pin);
    }

    tos::expected<tos::spbtle::fw_id, errors> get_fw_id() const;
    ...
    int spi_write(tos::span<const uint8_t> d1, tos::span<const uint8_t> d2);
...
private:
    tos::expected<void, errors> begin();
