...
public:
    ...
    explicit spi_sd_card(SpiT& spi, gpio_t& g, pin_t cs)
        : m_spi{&spi}
        , m_gpio{&g}
        , m_cs{cs} {
    }
...
private:
    spi_transaction<SpiT*> exec_cmd(uint8_t cmd, uint32_t arg, uint8_t crc = 0xFF) {
        ...
        spi::exchange(tr, cmd);
        spi::exchange(tr, arg >> 24); // MSB first
        spi::exchange(tr, arg >> 16);
        spi::exchange(tr, arg >> 8);
        spi::exchange(tr, arg);
        spi::exchange(tr, crc);
    ...
    uint8_t read_8(spi_transaction<SpiT*>&& tr) {
        ...
        for (int i = 0; i < 8; ++i) {
            auto x = force_get(spi::exchange(tr, 0xFF));
            if (x != 0xFF)
                ret = x;
        }
...
constexpr inline uint32_t get_blk_count(const csd_t& csd) {
    if (csd.v1.csd_ver == 0) {
        ...
        uint16_t c_size =
            (csd.v1.c_size_high << 10) | (csd.v1.c_size_mid << 2) | csd.v1.c_size_low;
...
template<class SpiT>
inline spi_sd_card<SpiT> open_impl(devs::sd_t /*sd*/,
                                   SpiT& spi,
                                   typename SpiT::gpio_type& g,
                                   typename SpiT::gpio_type::pin_type pin) {
...
bool spi_sd_card<SpiT>::init() {
    for (int i = 0; i < 10; ++i) {
        tos::spi::exchange(m_spi, 0xFF);
    }

    using namespace std::chrono_literals;
    int16_t i;
    for (i = 0; i < 10; ++i) {
        if (read_8(exec_cmd(0x40 + 0, 0x00000000, 0x95)) == 0x01)
            break;
        tos::delay_ms(100ms);
    }
    if (i == 10)
        return false;
    for (i = 0; i < 10; ++i) {
        if (read_8(exec_cmd(0x40 + 8, 0x000001AA, 0x87)) == 0xAA)
            break;
        tos::delay_ms(100ms);
    }
    if (i == 10)
...
void spi_sd_card<SpiT>::read(void* to, uint32_t blk, uint16_t len, uint16_t offset) {
    ...
    while (force_get(spi::exchange(tr, 0xFF)) != 0x00)
        ;
    while (force_get(spi::exchange(tr, 0xFF)) != 0xFE)
        ;

    uint16_t i;
    for (i = 0; i < offset; ++i) {
        tos::spi::exchange(tr, 0xFF);
    }

    auto target = reinterpret_cast<uint8_t*>(to);
    tr->exchange(tos::span<uint8_t>{target, len});
    i += len;
...
csd_t spi_sd_card<SpiT>::read_csd() {
    ...
    for (int i = 0; i < 16; ++i) {
        res_ptr[i] = force_get(spi::exchange(tr, 0xFF));
    }

    spi::exchange(tr, 0xFF);
    spi::exchange(tr, 0xFF);
    spi::exchange(tr, 0xFF);
    return res;
}
