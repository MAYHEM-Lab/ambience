...
namespace tos
{
    ...
    class bme280 : public self_pointing<bme280<I2CT, DelayT>>, public non_copyable, DelayT
    {
    public:
        bme280(twi_addr_t addr, I2CT i2c, DelayT delay) : m_i2c(std::move(i2c)), DelayT(std::move(delay)) {
            ...
            m_dev.read = [](uint8_t dev_id, uint8_t reg_addr, uint8_t *reg_data, uint16_t len, void* user) -> int8_t
            {
                auto self = static_cast<bme280*>(user);
        ...
        bme280(bme280&& rhs) : m_dev{rhs.m_dev}, m_i2c{std::move(rhs.m_i2c)} {
            m_dev.user = this;
        }
        ...
        void set_config()
        {
            ...
            rslt = bme280_set_sensor_mode(BME280_NORMAL_MODE, &m_dev);
        }
        ...
        tos::expected<bme280_data, read_errors> read()
        {
            bme280_data res;
            auto rslt = bme280_get_sensor_data(BME280_ALL, &res, &m_dev);
            if (rslt != BME280_OK)
            {
