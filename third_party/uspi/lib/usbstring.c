...
void USBString (TUSBString *pThis, struct TUSBDevice *pDevice)
{
	...
	pThis->m_pString = tos_malloc (sizeof (TString));
...
void USBStringCopy (TUSBString *pThis, TUSBString *pParent)
{
	...
	{
		pThis->m_pUSBString = (TUSBStringDescriptor *) tos_malloc (pParent->m_pUSBString->bLength);
	...
	pThis->m_pString = (TString *) tos_malloc (sizeof (TString));
	assert (pThis->m_pString != 0);
...
void _USBString (TUSBString *pThis)
{
	...
	tos_free (pThis->m_pString);
	pThis->m_pString = 0;

	if (pThis->m_pUSBString != 0)
	{
		tos_free (pThis->m_pUSBString);
		pThis->m_pUSBString = 0;
	}
...
boolean USBStringGetFromDescriptor (TUSBString *pThis, u8 ucID, u16 usLanguageID)
{
	...
	pThis->m_pUSBString = (TUSBStringDescriptor *) tos_malloc (USBSTR_MIN_LENGTH);
	assert (pThis->m_pUSBString != 0);
	...
	{
		tos_free (pThis->m_pUSBString);
		pThis->m_pUSBString = (TUSBStringDescriptor *) tos_malloc (ucLength);
		assert (pThis->m_pUSBString != 0);
...
u16 USBStringGetLanguageID (TUSBString *pThis)
{
	...
	TUSBStringDescriptor *pLanguageIDs = (TUSBStringDescriptor *) tos_malloc (USBSTR_MIN_LENGTH);
	...
	{
		tos_free (pLanguageIDs);
		pLanguageIDs = (TUSBStringDescriptor *) tos_malloc (ucLength);
		assert (pLanguageIDs != 0);
