...
void USBMouseDevice (TUSBMouseDevice *pThis, TUSBFunction *pDevice)
{
	...
	pThis->m_pReportBuffer = tos_malloc (MOUSE_BOOT_REPORT_SIZE);
...
void _CUSBMouseDevice (TUSBMouseDevice *pThis)
{
	...
	{
		tos_free (pThis->m_pHIDReportDescriptor);
		pThis->m_pHIDReportDescriptor = 0;
	}

	if (pThis->m_pReportBuffer != 0)
	{
		tos_free (pThis->m_pReportBuffer);
		pThis->m_pReportBuffer = 0;
	}
	...
	if (pThis->m_pReportEndpoint != 0)
	{
		...
		tos_free (pThis->m_pReportEndpoint);
		pThis->m_pReportEndpoint = 0;
	}
...
boolean USBMouseDeviceConfigure (TUSBFunction *pUSBFunction)
{
		...
		USBFunctionGetDescriptor (&pThis->m_USBFunction, DESCRIPTOR_ENDPOINT)) != 0)
	{
		...
		pThis->m_pReportEndpoint = tos_malloc (sizeof (TUSBEndpoint));
		assert (pThis->m_pReportEndpoint != 0);
	...
	pThis->m_pHIDReportDescriptor = (u8 *) tos_malloc (pThis->m_usReportDescriptorLength);
	...
	{
		LogWrite (FromUSBMouse, USPI_LOG_ERROR, "Cannot get HID report descriptor");
	...
	{
		LogWrite (FromUSBMouse, USPI_LOG_ERROR, "Cannot set interface");
	...
	{
		LogWrite (FromUSBMouse, USPI_LOG_ERROR, "Cannot set boot protocol");
