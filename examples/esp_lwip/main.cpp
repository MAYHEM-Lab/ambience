...
conn:
    ...
    with(
        ...
        [&](tos::esp82::wifi_connection& conn) {
            ...
            auto addr = with(
                conn.get_addr(),
                [&](auto& addr) { return addr; },
                [](auto) { return tos::ipv4_addr_t{}; });
            ...
            auto acceptor = [&](auto&, tos::esp82::tcp_endpoint&& newep) {
                ...
                ep =
                    new (mem) tos::tcp_stream<tos::esp82::tcp_endpoint>(std::move(newep));
                s.up();
                return true;
            };
            ...
            while (true) {
                ...
                with(
                    ...
                    [&](tos::read_error err) { tos::println(usart, "disconnected!"); });
