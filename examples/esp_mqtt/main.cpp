...
conn:
    ...
    with(
        std::move(res),
        [&](tos::esp82::wifi_connection& conn) ICACHE_FLASH_ATTR {
            conn.wait_for_dhcp();
            ...
            for (int j = 0; j < 25; ++j) {
                ...
                while (!isconn)
                    with(
                        tos::esp82::connect(conn, {{52, 59, 177, 21}}, {1883}),
                        [&](tos::esp82::tcp_endpoint& conn) {
                            tos::tcp_stream<tos::esp82::tcp_endpoint> stream{
                                std::move(conn)};
                            net_facade net{stream};
                            isconn = true;

                            auto client =
                                new MQTT::Client<net_facade, timer_facade, 512>(net);
                        ...
                        [&](auto&) { tos::println(usart, "couldn't connect"); });
            }

            tos::println(usart, "done");
        },
        [&](auto&) { tos::println(usart, "uuuh, shouldn't have happened!"); });
}
