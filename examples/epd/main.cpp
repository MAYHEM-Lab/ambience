...
public:
    ...
    explicit gde021a1(SpiT spi, PinT chip_select, PinT data_command, PinT reset, PinT busy)
        : m_spi{std::move(spi)}
        , m_cs{chip_select}
        , m_dc{data_command}
        , m_reset{reset}
    ...
    void set_display_window(const tos::gfx::point& pos, const tos::gfx::dimensions& dims) {
        using namespace gde021a1_constants;
...
private:
    ...
    void init() {
        ...
        gde021a1WriteRegArray(EPD_REG_50, waveform_lookup_table, std::size(waveform_lookup_table)); // Write LUT
    }
...
void blink_task() {
    ...
    gde021a1<tos::stm32::spi*> display(&spi, chip_select, data_command, 18_pin, 8_pin);
