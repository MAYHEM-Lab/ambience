...
struct bench_it {
  constexpr bool operator==(bench_range_end_t) { return m_i == m_end; }

  constexpr bool operator!=(bench_range_end_t) { return m_i != m_end; }

  constexpr int operator*() { return m_i; }
...
struct bench_range {
  ...
  constexpr bench_it begin() { return bench_it{m_len}; }
...
template <class T> auto bench(T &&t) {
  std::vector<std::chrono::microseconds> expr;
...
conn:
  ...
  auto acceptor = [](auto &, tos::esp82::tcp_endpoint &&ep) {
    tos::launch(
        tos::alloc_stack,
        [p = std::make_unique<tos::tcp_stream<tos::esp82::tcp_endpoint>>(
             std::move(ep))] { source_task(p); });
    return true;
  };
...
void tos_main() { tos::launch(tos::stack_size_t{8096}, esp_main); }
