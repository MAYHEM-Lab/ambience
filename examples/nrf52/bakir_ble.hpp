...
private:
    template <class LogT>
    friend std::unique_ptr<ble_service> make_ble_service(tos::nrf52::softdev& sd, LogT&& log);
...
template <class LogT>
inline std::unique_ptr<ble_service> make_ble_service(tos::nrf52::softdev& sd, LogT&& log)
{
    auto vs_reg_res = sd.register_vs_uuid(ble_service::uuid);
    if (!vs_reg_res) return nullptr;

    auto res = std::make_unique<ble_service>();
    auto uuid_handle = force_get(vs_reg_res);

    res->m_uuid = tos::nrf52::overlay_uuid(uuid_handle);
    res->m_uuid.set_overlay(1);

    auto err_code = sd_ble_gatts_service_add(BLE_GATTS_SRVC_TYPE_PRIMARY,
                                        &res->m_uuid.get(),
    ...
    add_char_params.uuid_type                = res->m_uuid.get().type;
